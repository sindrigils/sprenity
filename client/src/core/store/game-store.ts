...
import type { GridCell } from '@api/zones/requests';
...
export type ActiveHoverCell = {
  key: string;
  cell: GridCell;
  color: string;
};

export type ZoneBuildNotification = {
  id: string;
  message: string;
};

type GameStore = {
  agentsMap: Map<string, Agent3DState>;
...
export const useGameStore = create<GameStore>((set, get) => ({
  agentsMap: new Map<string, Agent3DState>(),
  ...
  registerAgent: (id: string, object: THREE.Object3D) => {
    set((state) => {
      ...
      newMap.set(id, { id, object, targetPosition: null });
  ...
  setActiveHoverCell: (cell: GridCell, color: string, nowMs: number) => {
    set((state) => {
      ...
      return {
        ...
        activeHoverCell: { key, cell, color },
      };
    });
  },
